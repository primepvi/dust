...
void update(float dt) {
    ...
    if (input_key_down(KEY_W)) movement.y -= 1.0f;
    if (input_key_down(KEY_S)) movement.y += 1.0f;
    if (input_key_down(KEY_D)) movement.x += 1.0f;
    if (input_key_down(KEY_A)) movement.x -= 1.0f;

    float speed = 200.0f;
    pos.x += movement.x * speed * dt;
    pos.y += movement.y * speed * dt;

    if (input_key_pressed(KEY_ESCAPE)) engine_quit();
}
...
void draw(void) {
    ...
    Mat4 proj = mat4_ortho(0.0f, (float)window_width(), (float)window_height(), 0.0f);
...
int main(void) {
    WindowConfig config = {
        ...
        .resizable = true, .title = "dust."
    };

    if (!engine_init(config)) return 1;
    ...
    program = shader_from_files("assets/shaders/main.vert", "assets/shaders/main.frag");
